...
// же кодом чистит отправитель. Он ES-модуль, поэтому подключается через
// import(), а не require.
let sharedCleaners = null;
function cleaners() {
    sharedCleaners = sharedCleaners
        || import(pathToFileURL(path.join(__dirname, '..', 'public', 'crypto', 'metadata.js')).href);
    return sharedCleaners;
}
...
const VIDEO_WITH_METADATA = new Set(['video/mp4', 'video/quicktime']);
...
async function stripMetadataFromFile(filePath, mimeType) {
    ...
    try {
        ...
        } else if (VIDEO_WITH_METADATA.has(mimeType)) {
