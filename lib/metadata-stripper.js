...
// Эти картинки чистятся без перекодирования — тем же кодом, что в браузере.
const CLEAN_IN_PLACE = new Set(['image/jpeg', 'image/png', 'image/webp', 'image/gif']);

/**
 * Картинка без перекодирования: пиксели и анимация те же. Если в EXIF
 * записан поворот, без EXIF картинка легла бы набок, — такую перерисовывает
 * sharp с поворотом (анимированную так не сохранить — отказ).
 */
async function stripImageInPlace(inputPath, outputPath, mimeType) {
    const { cleanImage } = await cleaners();
    const cleaned = cleanImage(await fs.promises.readFile(inputPath), mimeType);
    if (cleaned.orientation === 1) {
        await fs.promises.writeFile(outputPath, cleaned.bytes);
    } else if (cleaned.animated) {
        throw new Error('анимация с поворотом в EXIF не поддерживается');
    } else {
        await stripImageMetadata(inputPath, outputPath);
    }
    return outputPath;
}

/** Удаление метаданных из WebM: теги (координаты, камера), название, дата. */
async function stripWebmMetadata(inputPath, outputPath) {
    const { cleanWebm } = await cleaners();
    await fs.promises.writeFile(outputPath, cleanWebm(await fs.promises.readFile(inputPath)));
    return outputPath;
}
...
async function stripMetadataFromFile(filePath, mimeType) {
    ...
    try {
        if (CLEAN_IN_PLACE.has(mimeType)) {
            await stripImageInPlace(filePath, tempPath, mimeType);
        } else if (mimeType.startsWith('image/')) {
            await stripImageMetadata(filePath, tempPath);
        } else if (mimeType === 'video/webm') {
            await stripWebmMetadata(filePath, tempPath);
        ...
        log.error({ err: error }, '[MetadataStripper] Error processing file');
...
// Разбор чужого файла — в отдельном процессе, с пределом памяти и времени
// (lib/limited-process.js): файл, на котором парсер зациклился бы или съел
// всю память, роняет только свой процесс, а не сервер.
const CLEAN_TIMEOUT_MS = Number(process.env.CLEAN_TIMEOUT_MS) || 60000;
const CLEAN_MAX_HEAP_MB = Number(process.env.CLEAN_MAX_HEAP_MB) || 384;

function stripMetadataInWorker(filePath, mimeType) {
    const { runLimited } = require('./limited-process');
    return runLimited(path.join(__dirname, 'clean-file.js'), { filePath, mimeType },
        { timeoutMs: CLEAN_TIMEOUT_MS, maxHeapMb: CLEAN_MAX_HEAP_MB });
}

module.exports = {
    stripMetadataInWorker,
    stripImageMetadata,
    stripImageInPlace,
    stripWebmMetadata,
    stripPdfMetadata,
