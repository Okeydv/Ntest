...
/*
 * Пришёл ли запрос через скрытый сервис Tor. Tor-демон подключается к
 * серверу с этой же машины, поэтому у всех пользователей .onion один адрес
 * — 127.0.0.1, и лимиты по IP становились общими на всех. Такие запросы
 * лимиты считают по аккаунту и браузеру (lib/rate-limits.js), а сокеты —
 * по пользователю (lib/sockets.js).
 *
 * Одного заголовка Host мало — его подставит кто угодно и уйдёт от лимитов
 * по IP. Поэтому ещё и адрес должен быть локальным: снаружи с 127.0.0.1 не
 * прийти.
 */
const LOOPBACK = new Set(['127.0.0.1', '::1', '::ffff:127.0.0.1']);
const isOnionHost = host => String(host || '').split(':')[0].toLowerCase().endsWith('.onion');

function viaTor(req) {
    return isOnionHost(req.headers.host) && LOOPBACK.has(req.realIp || req.ip);
}

function handshakeViaTor(handshake) {
    return isOnionHost(handshake.headers && handshake.headers.host) && LOOPBACK.has(getClientIp(handshake));
}

module.exports = {
    isFromCloudflare, resolveRealIp, isAllowedOrigin, getClientIp, getLocalAddresses, viaTor, handshakeViaTor,
};
