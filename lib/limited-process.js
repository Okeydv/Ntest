...
// Что из окружения нужно дочернему процессу: пути, язык, журнал и пределы
// очистки. Остальное (DATABASE_URL, SESSION_SECRET, секрет сервера ключей…)
// не передаётся.
const CHILD_ENV_KEYS = ['PATH', 'HOME', 'LANG', 'LC_ALL', 'TMPDIR', 'NODE_ENV', 'LOG_LEVEL',
    'CLEAN_TIMEOUT_MS', 'CLEAN_MAX_HEAP_MB', 'NODE_PATH'];

function childEnv() {
    const env = {};
    for (const key of CHILD_ENV_KEYS) if (process.env[key] !== undefined) env[key] = process.env[key];
    return env;
}
...
function runOnce(script, data, { timeoutMs, maxHeapMb }) {
    return new Promise((resolve, reject) => {
        const child = fork(script, [], {
            ...
            // Без секретов сервера: разбор чужого файла — самое уязвимое
            // место, и попавший в процесс код не должен найти там ни пароля
            // к базе, ни ключа сессий.
            env: childEnv(),
...
module.exports = { runLimited, WorkerLimitError, childEnv };
