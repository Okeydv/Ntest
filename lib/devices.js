 ...
 */
function createDevicesRouter({ pool, dbGet, dbAll, dbRun, revokeDeviceKeys, onDeviceAdded = null, onDeviceRevoked = null }) {
    ...
    router.post('/api/devices', requireAuth, async (req, res) => {
        ...
        try {
            ...
            log.error({ err: error }, '[Devices] Registration error');
    ...
    router.post('/api/devices/:id/bind', requireAuth, async (req, res) => {
        ...
        try {
            ...
            log.error({ err: error }, '[Devices] Bind error');
    ...
    router.get('/api/devices', requireAuth, async (req, res) => {
        try {
            ...
            log.error({ err: error }, '[Devices] List error');
    ...
    router.delete('/api/devices/:id', requireAuth, async (req, res) => {
        ...
        try {
            const device = await dbGet(
                'SELECT id, name, revoked_at FROM devices WHERE id = $1 AND user_id = $2',
                [deviceId, req.session.userId]
            );
            if (!device) {
                return res.status(404).json({ success: false, message: 'Устройство не найдено' });
            ...
            if (!device.revoked_at) {
                ...
                if (onDeviceRevoked) await onDeviceRevoked(req.session.userId, device);
            }
            ...
            log.error({ err: error }, '[Devices] Revoke error');
