...
test-e2ee-sessions test-e2ee-local-data test-e2ee-qr test-device-link test-read-state test-two-tabs test-feed test-composer test-empty-room test-e2ee-ui test-ui-basics test-ui-dialogs"
if [ $# -gt 0 ]; then
    SELECTED=" $* "
else
