...
## Дизайн-система и удаление голосовых сообщений

### 🎙 Убрана отправка голосовых
- Кнопка записи была `disabled` с подписью «пока не реализованы», но
  отправить голосовое всё равно можно было вложением аудиофайла: сервер
  помечал любой `audio/*` как `message_type: 'audio'` и подставлял текст
  «Голосовое сообщение». Путь убран целиком — кнопка, `audio/*` в `accept`,
  `ALLOWED_MIME_TYPES`, `MIME_EXTENSIONS` и подпись
- Проверка magic bytes для `video/webm` сузилась до корректной EBML-сигнатуры:
  раньше ветка обслуживала и `audio/ogg`, поэтому допускала Ogg-сигнатуру —
  и Ogg-файл проходил валидацию под видом `video/webm`
- Рендер `<audio>` в истории оставлен: голосовые, отправленные раньше, никуда
  из базы не делись и должны проигрываться

### 🎨 Дизайн-система
- `public/style.css` переписан на токены: цвет, пять слоёв по тону, шкала
  радиусов, шкала типографики, тени, тайминги. Светлая тема переопределяет те
  же токены, компоненты про цвет не знают
- Добавлена светлая тема и переключатель; выбор хранится в `localStorage`,
  по умолчанию — системный. `public/theme.js` подключён в `<head>` и ставит
  тему до отрисовки (инлайновый скрипт запрещён CSP)
- Свой знак бренда (затмение) вместо эмодзи 💬, он же в favicon и на пустом
  экране. Полумесяц остался только у переключателя темы
- Эмодзи-иконки заменены SVG-спрайтом
- Иерархия шрифта разведена по размеру, весу, межстрочному и межбуквенному
  интервалу: 38 / 30 / 26 / 19 / 15, лейблы капслоком с трекингом
- Состояния полей: фокус и ошибка различимы, ошибка подсвечивает поле и
  подписывается под ним вместо исчезающего тоста
- Скелетоны списка чатов с круглой аватаркой и бегущим бликом
- Контраст текста приведён к WCAG AA в обеих темах
- `prefers-reduced-motion` отключает анимации

### 🐛 Исправлено попутно
- `.chat-info` был описан в CSS дважды глобально — в элементе списка чатов
  побеждало `display: flex` от шапки чата, и название с последним сообщением
  вставали в строку вместо колонки. Селекторы разведены по контексту
- Ошибка загрузки списка чатов больше не оставляет скелетон висеть навсегда
